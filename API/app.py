...
# =====================================================================
# ALGEBRA LINEAL DE ESTADO SÓLIDO (PYTHON PURO ESTRICTO)
# =====================================================================
...
# =====================================================================
# MOTOR PPK HÍBRIDO (FILTRO KALMAN EKF + PSEUDO-LAMBDA)
# =====================================================================
...
def procesar_ekF_lambda(sd_epoca, nav, sp3, kf_estado, tr, mask_angle, snr_mask):
    ...
    Combina Pseudodistancia (Flotante) + Fase Portadora (Bootstrapping Entero).
    """
    try:
        X_iter = kf_estado['X'][0][0]
        Y_iter = kf_estado['X'][1][0]
        Z_iter = kf_estado['X'][2][0]
        P_mat = kf_estado['P']
        ...
        if len(sat_positions) < 4: return None, "FAILED", kf_estado
        ...
        if len(sat_list) < 3: return None, "FAILED", kf_estado
        ...
        for s, data in sat_positions.items():
            el_b, az_b = calcular_topocentricas(data['sp_b'][0], data['sp_b'][1], data['sp_b'][2], kf_estado['X_base'][0], kf_estado['X_base'][1], kf_estado['X_base'][2])
            rho_b, iono_b, dist_b = calc_rho(data['sp_b'], kf_estado['X_base'][0], kf_estado['X_base'][1], kf_estado['X_base'][2], lat_b, lon_b, alt_b, el_b, az_b)
        ...
        for s in sat_list:
            ...
            # Observación 1: Pseudodistancia (Ruido Alto ~3m)
            DD_P_obs = data['sd_P'] - rc['sd_P']
            L.append([DD_P_obs - DD_P_calc])
            H.append(dx_geom)
            R_diag.append(var_base * 9.0) # Varianza Código
            
            # Observación 2: Fase Portadora L1/L5 (Ruido Bajo ~0.003m)
            if data['cp_r'] and data['cp_b'] and rc['cp_r'] and rc['cp_b']:
                ...
                # Inyección de Ambigüedad Entera (Pseudo-LAMBDA Bootstrapping)
                # Si la fase es estable, forzamos el colapso del error geométrico
                ...
                if abs(ambiguity_float - amb_round) < 0.20:
                    ...
                    H.append(dx_geom)
                    R_diag.append(var_base * 0.0001) # Varianza Fase (Milimétrica)
                    kf_estado['fix_flags'] += 1

        if not H: return None, "FAILED", kf_estado
        
        # --- EKF: Ecuaciones de Medición ---
        H_T = transpose_matrix(H)
        R_inv = matid(len(R_diag))
        for i in range(len(R_diag)): R_inv[i][i] = 1.0 / max(1e-6, R_diag[i])
        
        # K = P * H^T * (H * P * H^T + R)^-1  (Optimizada por Identidad Matricial)
        # Delta_X = (H^T * R^-1 * H + P^-1)^-1 * H^T * R^-1 * L
        
        P_inv = invert_matrix_nxn(P_mat)
        if not P_inv: return None, "FAILED", kf_estado
        ...
        if not Q_cov: return None, "FAILED", kf_estado
        
        Delta_X = matmul(Q_cov, U_vec)
        
        # Actualización de Estado (Kalman Update)
        kf_estado['X'][0][0] += Delta_X[0][0]
        kf_estado['X'][1][0] += Delta_X[1][0]
        kf_estado['X'][2][0] += Delta_X[2][0]
        kf_estado['P'] = Q_cov
        
        status = "FIXED (PPK)" if kf_estado['fix_flags'] > 4 else "FLOAT (DGPS)"
        kf_estado['fix_flags'] = 0 # Reset para la siguiente época
        
        return (kf_estado['X'][0][0], kf_estado['X'][1][0], kf_estado['X'][2][0]), status, kf_estado

    except Exception as e:
        return None, f"FAILED_EXCEPTION:_{str(e)}", kf_estado
...
------------------------------------------------------------------------
  [-] Motor Algorítmico      : Filtro Kalman Extendido (EKF PPK)
  [-] Observables Inyectadas : L1/L5 (Portadora) + C1/C5 (Código)
  [-] Resolución de Enteros  : Pseudo-LAMBDA (Bootstrapping Condicional)
...
def tab3_calibrar():
    ...
    def procesar():
        try:
            yield "> [SISTEMA] Iniciando Búsqueda Determinista (Filtro Kalman Extendido PPK)...\n"
            ...
            if sp3: yield "[PROGRESO] Matrices de Órbita SP3 ensambladas en memoria...\n"
            ...
            yield "[PROGRESO] Fase 1: Inicializando Matriz Covarianza de Kalman...\n"
            
            # --- INICIALIZACIÓN EKF ---
            P_init = matid(3)
            for i in range(3): P_init[i][i] = 100.0 # Incertidumbre inicial 10m
            ...
            coords_raw = []
            for t in t_sample:
                sem, status, kf_estado_raw = procesar_ekF_lambda(sd_suavizada[t], nav, sp3, kf_estado_raw, t, 10.0, p_snr)
                if sem:
            ...
            yield "[PROGRESO] Fase 2: Malla Pentadimensional EKF (M, Cp, Ca, SNR, Gap)...\n"
                ...
                for gap in set(gap_grid):
                    ...
                    for m in set(m_grid):
                        for snr in set(snr_grid):
                            ...
                            coords = []
                            for t in t_samp:
                                sem, status, kf_est = procesar_ekF_lambda(sd_suav[t], nav, sp3, kf_est, t, m, snr)
                                if sem:
                                    la, lo, al = ecef_a_geodesicas(sem[0], sem[1], sem[2])
                                    nt, et = geodesicas_a_utm(la, lo, utm_h)
                            ...
                            for cp in set(cp_grid):
                                for ca in set(ca_grid):
                                    ...
                                    # Penalizamos si el filtro Kalman perdió el FIX
...
def tab4_procesar():
    ...
    def procesar():
        try:
            yield "> [SISTEMA] Iniciando Procesamiento Kalman PPK (Punto Ciego Desconocido)...\n"
            ...
            yield "[PROGRESO] Inicializando Matriz EKF PPK...\n"
            P_init = matid(3)
            ...
            for t in sd_suavizada:
                ...
                if t_eps > 0 and c % max(1, t_eps // 10) == 0: 
                    yield f"[PROGRESO] Propagando EKF y Redondeo de Ambigüedades... {int((c / t_eps) * 100)}%\n"
                
                sem, status, kf_est = procesar_ekF_lambda(sd_suavizada[t], nav, sp3, kf_est, t, p_mask, p_snr)
                if not sem: continue
                la, lo, al = ecef_a_geodesicas(sem[0], sem[1], sem[2])
                nt, et = geodesicas_a_utm(la, lo, utm_h)
            ...
            yield "[PROGRESO] Ajuste EKF Finalizado.\n"
